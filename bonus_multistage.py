...
================================================================================
 ...
 Coordinate system: Earth-Centred Inertial (ECI)
   ...
   Axes: fixed in space (x toward vernal equinox, z toward North Pole)
   Launch site: equator on the +x axis
   r0 = (R_EARTH, 0, 0)
   v0 = (0, Ω·R_EARTH, 0)  ← surface velocity from Earth's rotation

 Forces:
   Gravity  : a_g = −GM/|r|³ · r          (always toward centre)
   Drag     : F_D = −½ Cd A ρ(h) |v_rel| v_rel   (h < 100 km only)
              v_rel = v − Ω×r  (relative to co-rotating atmosphere)
   Thrust   : F_T = T · thrust_hat        (direction varies by phase)

 Mass flow: ṁ = T / (Isp · g₀)   (Tsiolkovsky)

 Integration: explicit Euler, dt = 0.1 s inside atmosphere, 5 s in vacuum

 SIMPLIFICATIONS (documented)
 ────────────────────────────
 1. Equatorial orbit: ISS actually orbits at 51.6° inclination.
    A real mission departs from a high-latitude site (Baikonur, Kennedy).
    Launching from the equator into an equatorial orbit avoids the large Δv
    of a plane-change manoeuvre and keeps the simulation in 2D (x-y plane).

 2. Pre-programmed gravity turn: pitch angle follows a fixed altitude profile.
    Real rockets use active GNC feedback; here the profile is hard-coded.

 3. Stage 3 attitude manoeuvres (pitch / roll / yaw for ISS docking) are
    represented as changes in the thrust direction vector — rigid-body
    rotational dynamics are not simulated.

 4. ISS rendezvous is simplified to matching orbital velocity at the same
    altitude. Proximity operations (station-keeping, approach corridor,
    docking port alignment) are not simulated.

 5. Staging: Stage 2 ignites immediately after Stage 1 separation (no coast
    between stages 1 and 2). Stage 3 ignites at apogee.

 6. Rocket body: single cross-section Cd·A used throughout all phases.
================================================================================
"""
...
# Real rockets begin pitching very early (1–5 km) to build horizontal
# velocity efficiently. Finishing at 80 km matches Falcon-9-class behaviour.
...
# ============================================================
#
# Inspiration: Soyuz-type architecture, scaled for a 500 kg payload.
# Structural fraction ≈ 10 % of stage propellant mass (simplified).
#
...
# ============================================================
# SECTION 5 — EULER INTEGRATION — MULTISTAGE
# ============================================================
...
def simulate_multistage(dt_atm=0.1, dt_space=5.0):
    ...
    v = np.array([0.0, OMEGA_EARTH * R_EARTH, 0.0], dtype=float)  # equatorial rotation
    m = float(M_INITIAL)

    prop1 = float(S1_M_PROP)   # propellant remaining, Stage 1
    prop2 = float(S2_M_PROP)   # propellant remaining, Stage 2
    prop3 = float(S3_M_PROP)   # propellant remaining, Stage 3
    ...
    # Time at which the final orbit phase begins
    t_circ_done = None
    ...
    while t < T_MAX:
        ...
        # ── Time step ─────────────────────────────────────────────────────
        # Use 1 s for orbital phases (Hohmann coast + ISS orbit) to keep
        # Euler drift small over thousands of seconds.  dt_space is used
        # during the burn phases above atmosphere where accuracy matters less.
        if h < H_DRAG_CUTOFF:
            dt = dt_atm
        ...
        if phase == PHASE_S1 and prop1 > 0:
            ...
            if dm > prop1:
                dm = prop1
            t_hat   = thrust_hat(r, h, mode='gravity_turn')
            a_thrust = (T_cur / m) * t_hat
            prop1   -= dm
            m       -= dm
            ...
            # Circularisation: burn prograde until tangential speed reaches
            # local circular-orbit speed (v_tang >= v_circ at current altitude).
            ...
            if v_tang_now >= v_circ_now:
                pass   # transition fires below
            else:
                T_cur   = S3_THRUST
                mdot    = T_cur / (S3_ISP * G0)
                dm      = min(mdot * dt, prop3)
                t_hat   = thrust_hat(r, h, mode='prograde')
                a_thrust = (T_cur / m) * t_hat
                prop3   -= dm
                m       -= dm

        # ── Integration step ──────────────────────────────────────────────
        # Coast phases (no drag, no thrust) use the leapfrog (Störmer-Verlet)
        # scheme, which is symplectic and conserves energy for pure gravity.
        # All other phases use explicit Euler.
        if phase in (PHASE_COAS, PHASE_HOH, PHASE_ORB) and np.all(a_drag == 0) and np.all(a_thrust == 0):
            # Kick-drift-kick leapfrog
            a_r    = -(GM / r_norm**2) * r_hat       # gravity at r_n
            v_half = v + a_r * (dt / 2)              # half-kick
            r      = r + v_half * dt                 # full drift
            r2     = np.linalg.norm(r)
            a_r2   = -(GM / r2**2) * (r / r2)        # gravity at r_{n+1}
            v      = v_half + a_r2 * (dt / 2)        # half-kick
        else:
        ...
        if phase == PHASE_S1:
            if prop1 <= 0:
                phase = PHASE_S2
                m    -= S1_M_STRUCT   # jettison Stage 1 structure
                events.append((t, 'Stage 1 separation'))

        elif phase == PHASE_S2:
            if prop2 <= 0:
                phase = PHASE_COAS
                m    -= S2_M_STRUCT   # jettison Stage 2 structure
            ...
            # Apogee: radial velocity passes through zero (downward)
            ...
            if v_tang_now >= v_circ_now:
                # Parking orbit achieved — immediately apply Hohmann burn 1:
                # prograde impulse to raise apogee to ISS altitude (H_ISS).
                # Compute target speed for transfer ellipse perigee (here, now):
                r_park  = r_norm
                r_iss   = R_EARTH + H_ISS
                sma_hoh = (r_park + r_iss) / 2
                v_hoh1  = np.sqrt(GM * (2 / r_park - 1 / sma_hoh))  # speed at perigee of transfer
                dv_hoh1 = v_hoh1 - v_circ_now                        # Δv to add (prograde)
                t_hat_now = prograde_hat(r)
                v    += dv_hoh1 * t_hat_now        # instantaneous velocity change
            ...
            # Coast on transfer ellipse until apogee at ISS altitude.
            # Detect apogee: radial velocity crosses zero (downward).
            v_radial_hoh = np.dot(v, r_hat)
            h_now = (r_norm - R_EARTH) / 1e3
            if v_radial_hoh < 0 and h_now > H_ISS / 1e3 * 0.95:
                # At apogee of transfer orbit (~ISS altitude): apply Hohmann burn 2
                # (prograde impulse to circularise at ISS altitude).
                v_circ_iss  = np.sqrt(GM / r_norm)
                v_tang_hoh  = np.sqrt(max(np.linalg.norm(v)**2 - v_radial_hoh**2, 0.0))
                dv_hoh2     = v_circ_iss - v_tang_hoh     # Δv to circularise
            ...
            # Coast for one orbital period at ISS orbit to demonstrate stable orbit.
            T_actual = 2 * np.pi * np.sqrt(r_norm**3 / GM)   # local orbital period
    ...
    h_arr     = (np.linalg.norm(r_arr, axis=1) - R_EARTH) / 1e3   # km
    speed_arr = np.linalg.norm(v_arr, axis=1)                      # m/s
...
def print_mission_summary(data):
    ...
    # Stage burnout times and altitudes
    ...
    if mask_orb.any():
        ...
        idx_mid = len(r_orb) // 2   # mid-orbit sample
    ...
    # Propellant Δv theoretical (Tsiolkovsky)
    print()
